...
contract thesis{
    ...
    */
    ...
    function allow(address spender, uint256 amount) public returns (bool){
        ...
        emit allowance(msg.sender, spender, allowed[msg.sender][spender]);
        return true;
    ...
    */
    ...
    function adjustAllowed(address spender, int amount) public returns (bool){

        require (spender != address(0), "You can not do this");

        //If the amount is positive the allowance is increased. It is decreased otherwise. 
        if (amount > 0){
            allowed[msg.sender][spender] += uint256(amount);
        }else{
            allowed[msg.sender][spender] -= uint256(-amount);
        }
    ...
    /*
    * @desc Adjusts the amount of an address' funds an other address is allowed to spend. This function is used internaly by the contract
    * to recalculate the allowance amount after each transaction.
    * @param owner The address whose the amount of funds that are allowed to be spent is adjusted.
    * @param spender The address whose allowance to spend another address' funds is adjusted.
    * @param amount The amount by which the allowance will be adjusted as a signed integer.
    * @return Returns true if the execution is successful and false otherwise.
    */
    
    function internalAdjustAllowed(address owner, address spender, int amount) public returns (bool) {
    ...
    */
    ...
    function sendCoins(address target, uint256 amount) public returns (bool){
    ...
    */
    ...
    function indirectSendCoins(address owner, address target, uint256 amount) public returns (bool){

        require (amount <= allowed[owner][msg.sender], "You are not allowed to spend these coins");
        ...
        internalAdjustAllowed(owner, msg.sender, -int(amount));
    ...
    */
    ...
    function burn(uint256 amount) public returns (bool){
        ...
        require (balance[msg.sender] >= amount, "Not enough coins to burn");
    ...
    */
    ...
    function indirectBurn(address owner, uint256 amount) public returns (bool){

        require (amount <= allowed[owner][msg.sender], "You are not allowed to burn this much");
